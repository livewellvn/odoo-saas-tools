...
class AuthSignupHome(auth_signup.controllers.main.AuthSignupHome):
    ...
    def get_saas_domain(self):
        ...
        base_saas_domain = config.get_param(request.cr, SUPERUSER_ID,
                                            full_param)
        return base_saas_domain

    def do_signup(self, qcontext):
        values = dict(
            (key, qcontext.get(key)) for key in ('login', 'name', 'password'))
        ...
        if qcontext.get('dbname', False):
            ...
            assert re.match('[a-zA-Z0-9_.-]+$', qcontext.get(
                'dbname')), "Only letters or numbers are allowed in domain."
            assert db_exists == False, "Domain exists"
        assert re.match(
            "^.+\\@(\\[?)[a-zA-Z0-9\\-\\.]+\\.([a-zA-Z]{2,3}|[0-9]{1,3})(\\]?)$",
            values['email']), "Please enter a valid email address."
        assert any([k for k in
                    values.values()]), "The form was not properly filled in."
